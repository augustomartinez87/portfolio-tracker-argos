...
export default function PositionDetailModal({ open, onClose, position, trades, currency = 'ARS', mepRate = 1 }) {
  ...
  const displayValues = useMemo(() => {
    ...
    return {
      price: isARS ? position.currentPrice : (position.currentPrice / mepRate),
      avgPrice: isARS ? position.avgPrice : (position.costUSD / position.totalQuantity),
      invested: isARS ? position.totalCost : position.costUSD,
      valuation: isARS ? position.valuation : position.valuationUSD,
      result: isARS ? position.result : position.resultUSD,
      resultPct: position.resultPct,
      dailyResult: isARS ? position.dailyResult : position.dailyResultUSD,
      dailyResultPct: position.dailyResultPct
    };
  ...
  const invested = position?.totalCost || 0;
  const getColorClass = (value) => (value >= 0 ? 'text-success' : 'text-danger');
  const isPositionUnavailable = position && (!position.currentPrice || position.currentPrice === 0);
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-6">
            <div className="bg-background-tertiary/50 rounded-lg p-4 border border-border-primary">
              ...
              <p className="text-text-primary font-mono text-lg font-semibold">{position.totalQuantity}</p>
            </div>
