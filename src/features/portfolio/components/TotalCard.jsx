        ...
        <tbody>
          <tr>
            ...
            <td className={`text-center ${paddingX} ${paddingY} text-text-primary font-mono text-base font-medium whitespace-nowrap tabular-nums`}>
              {formatVal(totals.valuation, totals.valuationUSD)}
            </td>
            {/* Invertido */}
            {columnSettings.showInvertido && (
              <td className={`text-center ${paddingX} ${paddingY} text-text-secondary font-mono text-xs font-normal tabular-nums`}>
                {formatVal(totals.invested, totals.investedUSD)}
              </td>
            )}
            {/* P&L $ */}
            <td className={`text-center ${paddingX} ${paddingY} whitespace-nowrap tabular-nums`}>
              <span className={`font-mono font-semibold text-base ${totals.result >= 0 ? 'text-success' : 'text-danger'}`}>
                {formatVal(totals.result, totals.resultUSD)}
              </span>
            </td>
            {/* P&L % */}
            <td className={`text-center ${paddingX} ${paddingY}`}>
              <span className={`font-medium px-1.5 py-0.5 rounded text-sm tabular-nums inline-block ${(currency === 'ARS' ? totals.resultPct : totals.resultPctUSD) >= 0
                ? 'bg-success/10 text-success'
                : 'bg-danger/10 text-danger'
                }`}>
                <PercentageDisplay value={currency === 'ARS' ? totals.resultPct : totals.resultPctUSD} className="!text-current" iconSize="w-3 h-3" />
              </span>
            </td>
            {/* P&L Diario $ */}
            {columnSettings.showDiario && (
              <td className={`text-center ${paddingX} ${paddingY} whitespace-nowrap tabular-nums`}>
                <span className={`font-mono text-sm font-medium ${totals.dailyResult >= 0 ? 'text-success' : 'text-danger'}`}>
                  {formatVal(totals.dailyResult, totals.dailyResultUSD)}
                </span>
              </td>
            )}
            {/* P&L Diario % */}
            {columnSettings.showDiarioPct && (
              <td className={`text-center ${paddingX} ${paddingY}`}>
                <span className={`font-medium px-1.5 py-0.5 rounded text-xs tabular-nums inline-block ${(currency === 'ARS' ? totals.dailyResultPct : totals.dailyResultPctUSD) >= 0
                  ? 'bg-success/10 text-success'
                  : 'bg-danger/10 text-danger'
                  }`}>
                  <PercentageDisplay value={currency === 'ARS' ? totals.dailyResultPct : totals.dailyResultPctUSD} className="!text-current" iconSize="w-3 h-3" />
                </span>
              </td>
            )}
          </tr>
